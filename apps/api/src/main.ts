  ...
  try {
    ...
    const ssml = `
      <speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="zh-CN">
        <voice name="${process.env.GOOGLE_TTS_VOICE || 'yue-HK-Standard-B'}">
    ...
    const request = {
      ...
      voice: {
        languageCode: 'zh-CN',
        name: process.env.GOOGLE_TTS_VOICE || 'yue-HK-Standard-B'
      },
      audioConfig: {
        audioEncoding: 'MP3',
    ...
    return res.json({
      audio: audioBase64,
      mode: 'google',
      voice: process.env.GOOGLE_TTS_VOICE || 'yue-HK-Standard-B'
    });
