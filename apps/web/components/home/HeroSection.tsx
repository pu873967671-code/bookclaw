  ...
  return (
    <section className="min-h-screen pt-36 pb-20 px-12 max-w-7xl mx-auto grid grid-cols-2 gap-20 items-center">
      <div className="max-w-[560px]">
        <span className="inline-flex items-center gap-2 text-xs font-medium tracking-[0.08em] uppercase text-amber mb-7 animate-fade-up">
          <span className="w-5 h-px bg-amber" />
          AI-Powered TTS Platform
        </span>
        
        <h1 className="font-serif text-5xl sm:text-6xl lg:text-[76px] leading-[1.05] tracking-[-1.5px] text-ink mb-7 animate-fade-up [animation-delay:0.15s]">
          Books that<br/><em className="text-amber">speak</em> to you
        </h1>
        
        <p className="text-[17px] font-light leading-[1.7] text-ink-60 max-w-[420px] mb-11 animate-fade-up [animation-delay:0.3s]">
        ...
        <div className="flex gap-4 items-center animate-fade-up [animation-delay:0.45s]">
      ...
      <div className="flex justify-center items-center animate-fade-in [animation-delay:0.25s]">
          ...
          <div className="p-5">
            <div className="flex items-end gap-[3px] h-10 mb-4">
              {Array.from({ length: 25 }).map((_, i) => (
                <div key={i} className={`flex-1 rounded-[2px] ${i < 10 ? 'bg-amber' : 'bg-ink-10'}`} style={{ height: `${[30,50,20,70,45,85,35,65,25,80,40,60,30,75,45,55,20,90,35,70,50,40,65,25,80][i]}%`, animation: 'wave 1.2s ease-in-out infinite', animationDelay: `${i * 0.06}s` }} />
              ))}
            </div>
